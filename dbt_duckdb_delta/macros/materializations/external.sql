{% materialization external, adapter="duckdb", supported_languages=['sql', 'python'] %}
  ...
  {%- set allowed_formats = ['csv', 'parquet', 'json', 'delta'] -%}
  {%- if format -%}
...
-- create a view on top of the location
  {% call statement('main', language='sql') -%}
    ...
    {% elif format == 'delta' %}
      create or replace view {{ intermediate_relation }} as {{ compiled_code }};
    {% endif %}
  {%- endcall %}
  {% do store_relation('delta', intermediate_relation, location, format, config) %}
  {% if format == 'delta' %}
      {% set query %}
        create or replace view {{intermediate_relation}} as select * from delta_scan('{{ location }}')
      {% endset %}
      {% do run_query(query) %}
  {% endif %}
  ...
  {% if plugin_name is not none or glue_register is true and format != 'delta' %}
    {% if glue_register %}
